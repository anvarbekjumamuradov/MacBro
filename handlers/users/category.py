...
async def get_products_by_category(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"{category_name} kategoriyasidagi gadjetlar", reply_markup=markup)
    await ShopState.next()
@dp.message_handler(state=ShopState.category)
async def get_products_by_category(message: types.Message, state: FSMContext):
    category_name = message.text
    cat_id = db.get_category(name=category_name)[1]
    markup = make_products_markup(cat_id)
    await state.update_data({
        "cat_id": cat_id
    })
...
async def get_products_by_category(message: types.Message, state: FSMContext):
    category_name = message.text
    cat_id = db.get_category(name=category_name)[2]
    markup = make_products_markup(cat_id)
    await state.update_data({
        "cat_id": cat_id
    })
...
async def get_products_by_category(message: types.Message, state:FSMContext):
    category_name = message.text
    cat_id = db.get_category(name=category_name)[3]
    markup = make_products_markup(cat_id)
    await state.update_data({
        "cat_id": cat_id
    })
...
async def get_products_by_category(message: types.Message, state:FSMContext):
    category_name = message.text
    cat_id = db.get_category(name=category_name)[4]
    markup = make_products_markup(cat_id)
    await state.update_data({
        "cat_id": cat_id
    })
